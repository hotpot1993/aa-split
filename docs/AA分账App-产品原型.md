...
| 交互说明 | Tab切换保留各页状态；Tab3凸起按钮不选中任何Tab，点击弹出P30记账抽屉 |
...
|---|---|
| 功能定位 | 单笔账单全信息与状态管理 |
| 页面元素 | ①标题/金额/日期/群组/分类 ②凭证照片（可全屏、可查看原图）③垫付人与金额 ④参与人及其待付金额+已付状态 ⑤操作区：[催款]→P26 带此账单；[标记已付]→弹层选择"谁已付" ⑥[编辑]（标题/金额/日期/凭证，权限：垫付人或群主）⑦[删除]（二次确认，需权限） |
...
|---|---|
| 功能定位 | 我加入的所有群组 |
| 页面元素 | ①顶部搜索框（搜群组名/群内成员）→P60 ②群组卡片列表：头像、名称、成员数、未结清金额角标（如"3笔待结算"）、最近账单预览 ③右下角悬浮[＋创建群组]→P21 ④底部[邀请朋友]快捷入口 |
| 交互说明 | 卡片点击→P23；长按卡片→弹层（置顶/退出群组） |
| 空状态 | "你还没有群组 🎉 创建一个或邀请朋友开始AA吧" |
| 跳转 | 进入：P10；跳出：P21/P23/P60 |
...
| 页面元素 | ①方案摘要："只需N笔" ②转账步骤列表：付款人→收款人+金额+对应账单（点击可展开明细）③操作：[复制转账文案]（纯文本，如"张三 → 我 ¥55.00（今晚聚餐）"）/[生成收款码卡片]（保存图片，含收款人支付宝/微信收款码位）/ [开始催款]→P26 ④切换开关：[最少笔数模式 / 逐笔结算模式]（逐笔=按账单逐人结算，适合小规格群） |
| 交互说明 | 每次生成方案前计算全群净值；已全部结清时显示"✅全群已清账"；方案生成后任何"标记已付"都会实时重算 |
...
|---|---|
| 功能定位 | 群组信息与规则设置 |
| 页面元素 | ①群组信息编辑（头像/名称/简介）②默认分摊方式（均摊/自定义/按比例）③提醒设置（新账单@全部成员④默认免分摊人员清单）⑤[解散群组]（群主，二次确认，数据按规则保留/删除）⑥[删除我的所有数据] |
...
| 字段 | 金额（必填，数字键盘，支持¥/美金切换）· 标题 (≤30字) · 日期(默认今天) · 地点(选填) · 分类(餐饮/交通/住宿/购物/娱乐/其他) · 群组(必选) · 垫付人(默认我) · 参与者(默认群组全员，可改) · 分摊方式(默认群组默认规则) · 凭证照片(选填) · 定期账单开关(选填) |
...
|---|---|
| 功能定位 | 通知偏好 |
| 页面元素 | ①新账单提醒（开）②催款提醒（开）③定期账单提醒（开）④群组动态@我（开）⑤免打扰时段（默认22:00-08:00）⑥催款默认文案编辑 |
