...
| 项目 | 内容 |
|---|---|
| 功能定位 | 拉人进群：链接/二维码/账户名直加/无需注册直接添加 |
| 页面元素 | ①邀请链接（形如 `aafen://join/群码`）：复制、微信好友分享、朋友圈分享 ②邀请二维码：保存相册/分享（对方扫码→打开App→登录→确认加入弹窗）③**方式二：账户名直接搜索添加**：输入对方账户名→可搜索到→直接加入；**仅能添加已注册用户** ④**方式三：直接添加（无需注册，仅群主可见）**：群主只填对方名字即可把人记进账本，对方**不需要**已注册账号 ⑤已添加成员列表 |
| 交互说明 | 对方接受邀请后双方都收到P40通知。方式三添加的是「非注册成员」：名字 1–32 字符（首尾空格自动去除，不可含换行），单群上限 50 人，**群内不可重名**（新名字与群内全体成员的显示名比对，包含已退出的成员）；被添加的人不会收到任何通知、无法登录、也无法被搜索到，其他群成员照常收到「新成员加入」播报。非注册成员可以参与分摊与结算，与注册成员同权 |
...
|---|---|
| 功能定位 | 成员与权限管理 |
| 页面元素 | 成员列表（头像/昵称/账户名/加入时间/状态：正常、已退出）；动作：①添加成员→P22 ②移除成员（需群主权限，二次确认，提示"其未结清账单仍会保留"）③转让群主 ④退出群组 ⑤**改名称**（仅非注册成员）⑥**绑定到账户**（仅非注册成员，即「认领」，见下） |
| 非注册成员的展示 | 显示头像 + 名称 + 灰色「未注册」小标签，**不显示 `@账户名`**（它没有可用的账户名）。其余行为与注册成员完全一致：可被分摊、可垫付、可进免分摊名单、可进定期账单、可进催款名单、计入统计与人均 |
| 认领（绑定到账户） | 群主把一个非注册成员认定为群内某个真实账号。入口在非注册成员那一行的「绑定到账户」；点击后搜索并选择目标账户，二次确认里写明「将合并 N 笔账单、M 条历史结算」。确认后：该非注册成员的全部账单、结算记录、免分摊名单都转到目标账号名下；若目标账号还不在群里则自动加入；若同一账单里两人都有份额则**金额相加**（保持账单总额不变），已付状态取「两行都已付」才算已付；同名的两条成员记录合并为一条，加入时间取更早的。**认领不可撤销** |
| 疑似同一人的提示 | 群内出现同名成员时（一个非注册、一个注册），在成员管理页对应行提示「疑似同一人，去认领」。因为非注册成员禁止重名，同名即可精确判定为疑似同一人 |
| 交互说明 | 群主标记👑；仅群主可移除/转让/改名称/认领；**移除一个还有未结清账单的非注册成员时，二次确认里必须列出未结清金额**；退出群组前提示"尚有未结账单，退出不影响账单结算" |
...
> ⚠️ **v1.0.5 变更**：P51 独立设置页已删除，设置项**全部并入 P50「我的」主页**——新增「字体风格（行内值）/ 通知设置」行，与「账号安全 / 数据导出 / 关于我们」同卡片；退出登录保留在 P50 底部。具体变更见 [开发进度 §九](../docs/开发进度.md)。
