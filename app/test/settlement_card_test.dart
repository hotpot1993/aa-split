// 收款码卡片（需求：点击后把「一键智能结算」结果生成图片并保存本地）回归测试：
...
void main() {
  testWidgets('点击「收款码卡片」按钮：渲染卡片并尝试保存（无异常）', (tester) async {
    await _pump(tester, const SettlementScreen(groupId: 'g1'));

    // 结算方案已加载（饭友群有未结清账单 → 转账款存在）
    await tester.scrollUntilVisible(find.text('收款码卡片'), 200,
        scrollable: find.byType(Scrollable).first);
    expect(find.text('收款码卡片'), findsOneWidget);
    await tester.tap(find.text('收款码卡片'));
