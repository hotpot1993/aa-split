...
| 5 | [开发进度存档](./开发进度.md) | v1.8：UI 全量对齐 Demo / 字体系统 / 图标素材系统 / 真机修复记录 / **9 项需求修复轮 + 发版 SOP**；**第二十节：非注册成员（占位账号）全链路实现** | ✅ |
| 6 | [自定义图标素材库](./pic/) | **文件名 = 对应 emoji**；`powershell -ExecutionPolicy Bypass -File scripts\process-icons.ps1` 一键处理 | ✅ |
| 7 | [API 端点清单（自动生成）](./api-endpoints.generated.md) | 由 `scripts/sync-docs.mjs` 扫描控制器生成（49 端点，含鉴权/注释摘要）；`docs-sync.yml` 每次 master 推送自动刷新，勿手改 | ✅ |
| 8 | [领域词汇表](../CONTEXT.md) | **仓库根目录**：统一术语的标准叫法与应避免的叫法（非注册成员 / 占位账号 / 认领 / 参与人 …），产品讨论、代码注释与文档共用；只做词汇，不描述实现 | ✅ |
| 9 | [架构决策记录（ADR）](./adr/) | 记录难以逆转、且不看说明会觉得奇怪的取舍。现有 `0001-占位账号承载非注册成员`（含三条被否决方案的理由） | ✅ |
